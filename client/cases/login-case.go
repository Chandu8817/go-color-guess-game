...
func LoginCase(conn net.Conn) {
	inputs := storage.Scanner.Text()
	fmt.Sscan(inputs, &storage.Email, &storage.Password)
	...
	// _, err = conn.Write([]byte(storage.Method))
	// if err != nil {
	...
	if res != "" {
		...
		fmt.Println("Invalid storage.Email/password.")
	}
